...
const stats = [
  { label: 'Total Depositado', value: 'R$ 3.500,00', icon: ArrowDownToLine, color: '#1E9F96' },
  { label: 'Total Sacado', value: 'R$ 2.100,00', icon: ArrowUpRight, color: '#58B3AD' },
  { label: 'Apostas Realizadas', value: '142', icon: TrendingUp, color: '#F5A623' },
  { label: 'Bônus Recebidos', value: 'R$ 250,00', icon: Gift, color: '#FB7185' },
];
...
const inputCls = 'w-full bg-[#0F232D] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-[#1E9F96] transition-colors placeholder-gray-600';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map(s => (
          <div key={s.label} className="bg-[#163242] rounded-xl p-4 border border-white/15 flex items-start gap-3">
      ...
      <div className="rounded-xl p-5 border border-white/15 account-gradient-a">
        ...
        <div className="flex gap-3">
          <button onClick={onOpenDeposit} className="flex-1 sm:flex-none sm:px-8 py-2.5 rounded-xl text-white text-sm font-semibold transition-opacity hover:opacity-90 bg-[#1E9F96]">
            Depositar
          </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
        {/* Form — 3/5 */}
        <div className="xl:col-span-3 bg-[#163242] rounded-xl border border-white/15 overflow-hidden">
          <div className="px-5 py-4 border-b border-white/15 flex items-center gap-2">
            <User className="w-4 h-4 text-[#1E9F96]" />
          ...
          <div className="p-5 space-y-4">
            ...
            <button className="mt-2 px-6 py-2.5 rounded-lg text-white text-sm font-semibold transition-opacity hover:opacity-90 bg-[#1E9F96]">
              Salvar Alterações
            </button>
          </div>
        </div>
        ...
        <div className="xl:col-span-2 space-y-4">
          {/* Verification */}
          <div className="bg-[#163242] rounded-xl border border-white/15 overflow-hidden">
            <div className="px-5 py-4 border-b border-white/15 flex items-center gap-2">
              <BadgeCheck className="w-4 h-4 text-[#58B3AD]" />
            ...
            <div className="p-5 space-y-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 border ${item.done ? 'border-[#1E9F96] bg-[#1E9F96]' : 'border-white/20 bg-white/5'}`}>
                    {item.done && <span className="text-white text-[10px]">✓</span>}
                  </div>
                  <span className="text-gray-400 text-xs">{item.label}</span>
                </div>
              ))}
              <button className="mt-2 w-full py-2.5 rounded-lg text-sm font-semibold text-white transition-opacity hover:opacity-90 bg-[#58B3AD]">
                Verificar Agora
              </button>
            </div>
          </div>
      ...
      <div className="flex gap-1">
        ...
        {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
          <button key={p} onClick={() => onChange(p)}
            className="px-2.5 py-1.5 rounded transition-colors dyn-bg dyn-text" style={{ '--dyn-bg': p === page ? '#1E9F96' : 'rgba(255,255,255,0.05)', '--dyn-text': 'white' } as React.CSSProperties}>
            {p}
          </button>
        ))}
...
function DateFilter({ from, to, onFrom, onTo }: { from: string; to: string; onFrom: (v: string) => void; onTo: (v: string) => void }) {
  const dateInputCls = 'flex-1 min-w-0 sm:flex-none bg-[#0F232D] border border-white/10 rounded-lg px-2 py-1.5 text-white text-xs focus:outline-none focus:border-[#1E9F96] [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:cursor-pointer';
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 mb-4 pb-4 border-b border-white/15">
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white hover:opacity-90 flex-shrink-0 bg-[#1E9F96]">
          Filtrar
        </button>
      </div>
    </div>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { label: 'Saldo Atual',      value: 'R$ 1.200,00', color: '#1E9F96' },
          { label: 'Total Depositado', value: 'R$ 3.500,00', color: '#58B3AD' },
          { label: 'Total Sacado',     value: 'R$ 2.100,00', color: '#F5A623' },
        ].map(c => (
          <div key={c.label} className="bg-[#163242] rounded-xl p-4 border border-white/15">
      ...
      <div className="flex gap-3">
        <button className="flex-1 py-2.5 rounded-xl text-white text-sm font-semibold hover:opacity-90 bg-[#1E9F96]">
          Depositar
        </button>
        ...
        <div className="flex border-b border-white/15">
          {(['depositos', 'saques'] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setPage(1); }}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === t ? 'text-white border-b-2 border-[#1E9F96]' : 'text-gray-400 hover:text-white'}`}>
            ...
            <tbody className="divide-y divide-white/5">
              {rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="py-3">
                    <span className="inline-flex items-center gap-1 text-[#1E9F96] text-xs">
...
function Apostas() {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total Apostado', value: 'R$ 73,70', color: '#FB7185' },
          { label: 'Total Ganho',    value: 'R$ 74,50', color: '#1E9F96' },
          { label: 'Lucro/Perda',    value: '+R$ 0,80', color: '#1E9F96' },
        ].map(c => (
          <div key={c.label} className="bg-[#163242] rounded-xl p-4 border border-white/15">
            <p className="text-gray-500 text-xs mb-1">{c.label}</p>
      ...
      <div className="bg-[#163242] rounded-xl border border-white/15 overflow-hidden">
        <div className="flex border-b border-white/15 overflow-x-auto [scrollbar-width:none]">
          {tabs.map(t => (
            <button key={t} onClick={() => { setTab(t); setPage(1); }}
              className={`flex-shrink-0 px-5 py-3 text-sm font-medium capitalize transition-colors ${tab === t ? 'text-white border-b-2 border-[#1E9F96]' : 'text-gray-400 hover:text-white'}`}>
              ...
              <tbody className="divide-y divide-white/5">
                {rows.map((b, i) => (
                  <tr key={i}>
                    ...
                    <td className="py-3">
                      <span className={`text-xs font-semibold ${b.status === 'Ganhou' ? 'text-[#1E9F96]' : 'text-red-400'}`}>{b.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function Limites() {
  ...
  return (
    <div className="space-y-3">
      <div className="bg-[#163242] rounded-xl p-4 border border-[#58B3AD]/30 flex gap-3">
        <AlertCircle className="w-4 h-4 text-[#58B3AD] flex-shrink-0 mt-0.5" />
      ...
      {sections.map(s => (
        <div key={s.id} className="bg-[#163242] rounded-xl border border-white/15 overflow-hidden">
          <button
          ...
          >
            <div className="flex items-center gap-3">
              ...
              >
                {enabled[s.id]
                  ? <ToggleRight className="w-5 h-5 text-[#1E9F96]" />
            ...
            <div className="px-5 pb-5 border-t border-white/15 pt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    className="w-full bg-[#0F232D] border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#1E9F96]"
                    placeholder="Ex: 10"
                  />
                </div>
                <div>
              ...
              <button className="mt-4 px-5 py-2 rounded-lg text-white text-sm font-semibold hover:opacity-90 bg-[#1E9F96]">
                Salvar Limite
              </button>
            </div>
          )}
...
function Seguranca() {
  ...
  return (
    <div className="space-y-5 max-w-lg">
      <div className="bg-[#163242] rounded-xl border border-white/15 overflow-hidden">
        <div className="px-5 py-4 border-b border-white/15 flex items-center gap-2">
          <Lock className="w-4 h-4 text-[#58B3AD]" />
        ...
        <div className="p-5 space-y-4">
            ...
            <div key={f.label}>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#0F232D] border border-white/10 rounded-lg px-3 py-2.5 pr-10 text-white text-sm focus:outline-none focus:border-[#1E9F96]"
                  placeholder="••••••••"
                />
          ...
          <button className="w-full py-2.5 rounded-lg text-white text-sm font-semibold hover:opacity-90 bg-[#58B3AD]">
            Atualizar Senha
          </button>
        </div>
      </div>
        ...
        <div className="divide-y divide-white/5">
          {[
            ...
            <div key={i} className="px-5 py-3 flex items-center justify-between gap-4">
              ...
              <CheckCircle2 className="w-4 h-4 text-[#1E9F96] flex-shrink-0" />
            </div>
          ))}
        </div>
      </div>
      ...
      >
            ...
            {reasons.map(r => (
              <label key={r} className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 accent-[#1E9F96] flex-shrink-0"
                />
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-white/15">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity bg-[#1E9F96]"
          >
            Sim, continuar
          </button>
        </div>
...
function PausasSuspensoes({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <button onClick={onBack} className="flex items-center gap-2 text-[#58B3AD] font-bold text-lg hover:text-[#7DD9D2] transition-colors">
      ...
      <div className="flex gap-6 border-b border-white/15">
        <button
          ...
          className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
            tab === 'pausa' ? 'text-white border-[#1E9F96]' : 'text-gray-500 border-transparent hover:text-gray-300'
          }`}
        >
          Pausa nas Apostas
        </button>
        <button
          ...
          className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
            tab === 'suspensao' ? 'text-white border-[#1E9F96]' : 'text-gray-500 border-transparent hover:text-gray-300'
          }`}
        >
          Suspensão da Conta
        </button>
      ...
      <div className="rounded-xl border border-white/15 overflow-hidden account-gradient-b">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            {activeConfirmed && (
              <div className="rounded-xl p-4 border border-[#1E9F96]/30 flex items-start gap-3 bg-[#1E9F9615]">
                <CheckCircle2 className="w-4 h-4 text-[#1E9F96] flex-shrink-0 mt-0.5" />
              ...
              <>
                  ...
                  {Object.keys(PRESET_DAYS).map(label => (
                    <button
                      ...
                      className={`text-left py-3 px-4 rounded-xl text-sm border transition-all ${
                        days === PRESET_DAYS[label] ? 'border-[#1E9F96] bg-[#1E9F96]/10' : 'border-white/15 hover:border-white/30 bg-[#0F232D]/60'
                      }`}
                    >
                      <span className="block text-white font-medium">Pausar apostas por</span>
                      <span className={`block text-xs mt-0.5 ${days === PRESET_DAYS[label] ? 'text-[#1E9F96]' : 'text-gray-500'}`}>{label}</span>
                    </button>
                  ))}
                </div>
                ...
                <div className="pt-2">
                  ...
                  <input
                    ...
                    className="w-full accent-[#1E9F96]"
                  />
                  <div className="mt-3 inline-flex flex-col items-start">
                    <span className="text-gray-500 text-[11px] mb-1">Mínimo</span>
                    <span className="bg-[#0F232D] border border-white/15 rounded-lg px-3 py-1 text-white text-sm font-semibold">{days}</span>
                  </div>
                </div>
              </>
            ) : (
                ...
                <div className="grid grid-cols-2 gap-3">
                  {suspensaoOptions.map(label => (
                    <button
                      ...
                      className={`text-left py-3 px-4 rounded-xl text-sm border transition-all ${
                        suspensaoPeriodo === label ? 'border-[#1E9F96] bg-[#1E9F96]/10' : 'border-white/15 hover:border-white/30 bg-[#0F232D]/60'
                      }`}
                    >
                      <span className="block text-white font-medium">Suspender</span>
                      <span className={`block text-xs mt-0.5 ${suspensaoPeriodo === label ? 'text-[#1E9F96]' : 'text-gray-500'}`}>{label}</span>
                    </button>
                  ))}
                </div>

                <button className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium text-gray-300 border border-white/15 hover:border-white/30 bg-[#0F232D]/60 transition-colors">
            ...
            <button
              ...
              className="w-full py-3 rounded-xl text-white text-sm font-semibold disabled:opacity-40 transition-opacity hover:opacity-90 bg-[#1E9F96]"
            >
              Continuar
            </button>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            <select
              ...
              className="w-full bg-[#0F232D] border border-white/15 rounded-xl px-4 py-3.5 text-white text-sm focus:outline-none focus:border-[#1E9F96] appearance-none pr-10"
            >
          ...
          <button
            ...
            className="px-8 py-3.5 rounded-xl text-white text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-60 flex items-center justify-center gap-2 flex-shrink-0 bg-[#1E9F96]"
          >
    ...
    <div className="space-y-4 max-w-2xl">
      <div className="bg-[#163242] rounded-xl border border-white/15 overflow-hidden">
        <div className="px-5 py-4 border-b border-white/15 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Landmark className="w-4 h-4 text-[#58B3AD]" />
            <h3 className="text-white font-semibold text-sm">Suas Contas PIX</h3>
          </div>
          <button
            onClick={() => setShowForm(p => !p)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white hover:opacity-90 transition-opacity bg-[#1E9F96]"
          >
        ...
        {showForm && (
          <div className="px-5 py-4 border-b border-white/15 bg-[#0F232D] flex gap-3 flex-wrap">
            <div className="flex-1 min-w-[160px]">
              <label className="text-gray-500 text-xs mb-1 block">Chave PIX</label>
              <input
                className="w-full bg-[#163242] border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#1E9F96]"
            ...
            <div>
              ...
              <select
                className="bg-[#163242] border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#1E9F96]"
                value={newTipo}
            ...
            <div className="flex items-end gap-2">
              <button onClick={add} className="px-4 py-2 rounded-lg text-white text-sm font-semibold hover:opacity-90 bg-[#1E9F96]">Salvar</button>
    ...
    <div className="min-h-screen flex flex-col">
      <div className="flex-1">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5">
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-gradient-to-br from-[#1F475F] to-[#58B3AD] rounded-full flex items-center justify-center text-white text-xl font-bold flex-shrink-0">
                  GA
                </div>
                <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="mb-6 bg-[#163242] rounded-xl border border-white/15 p-1">
            <div
              ref={navScrollRef}
