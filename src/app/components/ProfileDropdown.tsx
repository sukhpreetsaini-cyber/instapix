  ...
  return (
    <div className={`relative ${isSidebar ? 'w-full' : ''}`} ref={ref}>
      <button
        ...
        className={`flex items-center gap-2 bg-white/5 px-2 sm:px-3 rounded-xl hover:bg-white/10 border border-white/10 h-[46px] ${isSidebar ? 'w-full' : ''}`}
      >
        <div className="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-[#1F475F] to-[#58B3AD] rounded-full flex items-center justify-center text-white text-xs sm:text-sm font-semibold flex-shrink-0">
          GA
        </div>
        {!compact && (
          <div className="text-left flex-1 min-w-0 hidden lg:block">
            <div className="text-white text-sm truncate">GA, Apostador</div>
          </div>
        )}
        {!compact && (
          <ChevronRight className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 transition-transform flex-shrink-0 ml-auto ${open ? 'rotate-90' : ''}`} />
        )}
      </button>

      {open && (
        <div className={`absolute bg-[#0F232D] rounded-xl shadow-2xl overflow-hidden z-[80] border border-white/10 ${isSidebar ? 'left-full top-0 ml-2 w-72' : 'right-0 top-full mt-2 w-72'}`}>
