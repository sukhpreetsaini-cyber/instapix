...
import { Home, Goal, Gamepad2, Layers, Video, Grid3x3, Search } from 'lucide-react';
...
export function HomeCategoryBar({ onNavigate }: HomeCategoryBarProps) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 mt-3 mb-5">
      <div className="inline-flex items-center gap-1.5 overflow-x-auto scrollbar-hide bg-[#0F242F] border border-white/8 rounded-xl px-2 h-[50px] min-w-0 max-w-full">
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
                isActive
                  ? 'bg-[#16323F] border border-[#2A5A6E] text-[#58B3AD]'
      ...
      <div className="hidden lg:flex items-center gap-2.5 bg-[#0F242F] border border-white/8 rounded-xl px-3.5 h-[50px] w-full max-w-xs flex-shrink-0">
